<template>
  <div class="container mx-auto px-4 py-8">
    <h2 class="text-3xl font-bold text-[#53b17b] mb-6">司機訂單管理</h2>
...
const fetchDriverOrders = async () => {
  try {
    const res = await api.getDriverBookings()
    orders.value = res.data
  } catch (error) {
    console.error('取得司機訂單列表失敗：', error)
  } finally {
    loading.value = false
  }
}
...
const toggleAvailability = async () => {
  try {
    ...
    await api.updateDriverAvailability({ isAvailable: isAvailable.value })
    // 顯示提示訊息或通知
  } catch (error) {
